({
          ...
          component.get('v.products').forEach(function (product) {
            ...
            if (product.name === 'e_sign' && product.status === 'active') {
              ...
              productRoles.e_sign.forEach(function (role) {
                roles.push({'label': helper.capitalizeRoles(role), 'value': helper.capitalizeRoles(role)});
              });
              product.roles = roles;
  ...
  setSortParams: function (component) {
    var userSortParams = {
      sortedBy: 'name', sortedDirection: 'desc'
    };

    var addUserSortParams = {
      sortedBy: 'name', sortedByLabel: $A.get('$Label.c.NameLabel'), sortedDirection: 'asc'
    };
  ...
  setAddUserColumns: function (component) {
    component.set('v.addUserColumns', [{
      label: $A.get('$Label.c.FirstName'), fieldName: 'FirstName', type: 'text', sortable: true
    }, {
      label: $A.get('$Label.c.LastName'), fieldName: 'LastName', type: 'text', sortable: true
    }, {
      label: $A.get('$Label.c.EmailAddress'), fieldName: 'Email', type: 'text', sortable: true
    }, {
      label: $A.get('$Label.c.ProfileLabel'), fieldName: 'ProfileName', type: 'text', sortable: true
    }]);
  },

  setFilterOptions: function (component) {
    component.set('v.filterOptions', [{
      disabled: true, label: $A.get('$Label.c.User')
    }, {
      disabled: false, label: $A.get('$Label.c.ProfileLabel')
    }, {
      disabled: false, label: $A.get('$Label.c.PermissionSetLabel')
    }]);
  },
  ...
  buildUsersTable: function (component, event, helper) {
    ...
    filteredUsers.forEach(function (user) {
      userRows.push({
        id: user.id.value,
        name: user.name,
        email: user.email, //TODO setup user status right now is being returned as blank from getUsers. Added a check currently so that if the status is blank
        // but the id and canManageAccount are valid indicates this as the setup user and the status is shown as Active
        status: !$A.util.isUndefinedOrNull(user.status) ? user.status.toString() : ($A.util.isUndefinedOrNull(user.status) && !($A.util.isUndefinedOrNull(user.id) && user.canManageAccount)) ? 'Active' : '',
        admin: user.canManageAccount,
        eSignatureRole: (!$A.util.isUndefinedOrNull(user.roles) && !$A.util.isEmpty(user.roles.e_sign)) ? user.roles.e_sign : null,
        documentGenerationRole: (!$A.util.isUndefinedOrNull(user.roles) && !$A.util.isEmpty(user.roles.gen)) ? user.roles.gen : null,
        negotiationRole: (!$A.util.isUndefinedOrNull(user.roles) && !$A.util.isEmpty(user.roles.negotiate)) ? user.roles.negotiate : null,
        clmRole: (!$A.util.isUndefinedOrNull(user.roles) && !$A.util.isEmpty(user.roles.clm)) ? user.roles.clm : null,
  ...
  setEsignContextColumns: function (component) {
    var actions = [{
      label: $A.get('$Label.c.EditPermissions'),
      name: 'edit_permissions'
    }, {label: $A.get('$Label.c.RemoveAndClose'), name: 'remove_close'}];

    //Set the columns for the data table
    component.set('v.dataColumns', [{
      sortable: true, label: $A.get('$Label.c.NameLabel'), type: 'text', fieldName: 'name'
    }, {
      sortable: true, label: $A.get('$Label.c.EmailAddress'), type: 'text', fieldName: 'email'
    }, {
      sortable: true, label: $A.get('$Label.c.Status'), type: 'text', fieldName: 'status'
    }, {
      sortable: false, label: $A.get('$Label.c.Admin'), type: 'boolean', fieldName: 'admin'
    }, {
      sortable: false, label: $A.get('$Label.c.TabESignature'), type: 'text', fieldName: 'eSignatureRole'
    }, {
      sortable: false,
  ...
  invokeAddUsers: function (component, event, helper) {
    ...
    action.setParams({
      users: JSON.stringify(usersToAdd), roles: JSON.stringify(rolesMap)
    });
    ...
    action.setCallback(this, function (response) {
      ...
      if (state === 'SUCCESS') {
        ...
        helper.refreshUsers(component, response.getReturnValue(), true);
      } else {
        helper.showToast(component, stringUtils.getErrorMessage(response), 'error');
        component.set('v.addUsersLoading', false);
        component.find('add-users').hide();
  ...
  prepareRolesMap: function (component) {
    var isAdmin = component.get('v.canManageAccount') === true;
    var rolesMap = {
      e_sign: isAdmin ? 'Administrator' : 'User'
    };
    if (component.get('v.context') === 'e_sign') {
      if (!$A.util.isEmpty(component.get('v.genRole'))) {
        rolesMap.gen = component.get('v.genRole');
      }
      if (!$A.util.isEmpty(component.get('v.negotiateRole'))) {
        rolesMap.negotiate = component.get('v.negotiateRole');
      }
    } else if (component.get('v.context') === 'clm') {
      //If context is e sign then add the default e sign user role
      if (isAdmin) {
        rolesMap.clm = 'Administrator';
      } else if (!$A.util.isEmpty(component.get('v.clmRole'))) {
        rolesMap.clm = component.get('v.clmRole');
      } else {
        rolesMap.clm = 'User';
      }
    }
  ...
  refreshUsers: function (component, updatedUsers, areNew) {
    component.set('v.userTableLoading', true);
    var users = component.get('v.users');
    if ($A.util.isEmpty(users)) {
      component.set('v.users', updatedUsers);
    } else if (!$A.util.isEmpty(updatedUsers)) {
      if (areNew) {
        // append and sort
        users = users.concat(updatedUsers);
        this.sortData(component, users, component.get('v.userSortParams'));
      } else {
        // Replace existing users with any that have been updated.
        for (var i = 0; i < users.length; i++) {
          for (var j = 0; j < updatedUsers.length; j++) {
            if (users[i].sourceId === updatedUsers[j].sourceId) {
              users.splice(i, 1, updatedUsers[j]);
              break;
            }
          }
        }
  ...
  invokeEditUserPermissions: function (component, event, helper) {
    ...
    editPermissionsAction.setParams({
      usersIdsToEdit: JSON.stringify(usersIdsToEdit), roles: JSON.stringify(rolesMap)
    });
    ...
    editPermissionsAction.setCallback(this, function (response) {
      ...
      if (state === 'SUCCESS') {
        ...
        helper.refreshUsers(component, response.getReturnValue(), false);
      } else {
        helper.showToast(component, stringUtils.getErrorMessage(response), 'error');
        component.set('v.editUsersLoading', false);
        component.find('edit-permissions').hide();
