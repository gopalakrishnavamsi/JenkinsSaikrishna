({
  ...
  handleSearchField: function (component) {
    ...
    if (queryTerm.length > 1) {
      component.set('v.allObjectFiledsList', allObjs.filter(function (obj) {
        return (obj.objectName.toLowerCase().includes(queryTerm.toLowerCase()));
      }));
    } else {
  ...
  onObjFieldSelection: function (component, event) {
    ...
    component.set('v.SelectedFiledsDetails', allObjects.filter(function (data) {
  ...
  onObjFieldDeSelection: function (component, event) {
    var apiName = event.currentTarget.id;
    var SelectedObjDetais = component.get('v.SelectedObjDetais');
    var allObjects = component.get('v.allObjectFileds');
    var allObjectsList = component.get('v.allObjectFiledsList');
