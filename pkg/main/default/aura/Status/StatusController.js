({
  ...
  toggleRecipientDetails: function(component, event) {
    ...
    for (var i = 0; i < recipients.length; i++) {
      ...
      if (
        parseInt(element.dataset.envelopeIndex, 10) === targetEnvelopeIndex &&
        parseInt(element.dataset.index, 10) === targetIndex
      ) {
  ...
  toggleEnvelopeDetails: function(component, event) {
    ...
    for (var i = 0; i < envelopes.length; i++) {
      if (
        parseInt(envelopes[i].getElement().dataset.index, 10) === targetIndex
      ) {
