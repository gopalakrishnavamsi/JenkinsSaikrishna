({
  ...
  validate: function (component) {
    return new Promise($A.getCallback(function (resolve, reject) {
      ...
      config.objectMappings.forEach(function (objMapping) {
        objMapping.fieldMappings = objMapping.fieldMappings.filter(function (fieldMapping) {
          if (fieldMapping.isChildRelation) {
            fieldMapping.childFieldMappings = fieldMapping.childFieldMappings.filter(function (childMapping) {
  ...
  removeSignerField: function (component, event) {
    ...
    if (fieldMappings.length == 0) {
      ...
      mappingComponents = Array.isArray(mappingComponents) ? mappingComponents : [mappingComponents];
