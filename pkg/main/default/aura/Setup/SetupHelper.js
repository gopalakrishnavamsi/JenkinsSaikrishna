({
  getState: function(component) {
    var self = this;
    this.invokeAction(component, component.get('c.getLogin'), null, function(
      login
    ) {
  ...
  goToStep: function(component, helper, stepName, showNextSteps) {
    ...
    if (stepName === 'setupAccount') {
      component.set(
        'v.nextStep',
        steps[1].status === 'complete' ? 'landing' : 'setupUsers'
      );
      ...
      component.set(
        'v.nextStep',
        steps[2].status === 'complete' ? 'landing' : 'setupLayouts'
      );
