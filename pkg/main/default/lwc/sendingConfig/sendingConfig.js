...
import { LABEL, PROGRESS_STEP, MIN_STEP, MAX_STEP, STEPS, OPERATION, getDocumentsForSending, getRecipientsForSending } from 'c/sendingUtils';
...
export default class SendingConfig extends LightningElement {
    ...
    */
    ...
    deleteSCMDocument() {
        deleteDocument({ scmFile: isEmpty(this.files) ? null : this.files[0] })
    ...
    navigateToTagger(envelope) {
        getTaggerUrl({
            ...
            .then(result => window.navUtils.navigateToUrl(result))
            .catch(error => showError(this.context, error, ERROR));
    }
