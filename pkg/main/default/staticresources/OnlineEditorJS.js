...
function initOnlineEditor() {
  ...
  Elements = Object.freeze({
    spinner1: j$('#ds-spinner'),
    spinner2: j$('#ds-spinner2'),
    buttons: {
      cancelPublish: j$('#onlineEditorPublishCancel'),
      cancelTemplate: j$('#onlineEditorTemplateCancel'),
      publish: j$('#onlineEditorPublish'),
      saveClose: j$('#onlineEditorSaveClose')
    }
  });

  if (Configuration.template) {
    j$('#fileNameInput').val(Configuration.template.fileName);
    if (Configuration.template.fileSuffix) j$('#fileSuffix').val(Configuration.template.fileSuffix);
  }

  _editor = new DSEditor({
    api: {
  ...
  createUserEventsComponent()
    .then(function (component) {
      ...
      if (navUtils.isIE()) j$('#IEWarning').show();
  ...
  Elements.buttons.publish.click(function () {
    onlineEditorPublish(Configuration.template, j$('#onlineEditorButtonNameInput').val())
      .then(function () {
  ...
  j$('#onlineEditorEdit').click(function () {
...
//Step 1
  j$('#onlineEditorStepLink1').click(function () {
    hideAll();
    editOnlineEditorTemplateDetails();
  });

//Step 2
  j$('#onlineEditorStepLink2').click(function () {
    ...
    navStep.onStart();
    if (!j$('#layoutCheckboxes').is(':empty')) {
      navStep.onEnd();
    } else {
...
function hideAll() {
  j$('#onlineEditorGenerator').hide();
  j$('#onlineEditor').hide();
  j$('#topPanel').hide();
  j$('#onlineEditorToolBar').hide();
  j$('#inputFilePanel').hide();
  j$('#namingHelp').hide();
}

function displayStartupElements() {
  j$('.hide-on-start-up').removeClass('hide-on-start-up');
}
...
function onlineEditorSaveClose(templateId, springTemplateId, isEditing, templateName) {
  ...
  exportDocument()
    ...
    .then(function (layouts) {
      if (!j$('#layoutCheckboxes').is(':empty')) return null;
...
function onlineEditorPublish(template, buttonLabel) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    updateFileDetailsInSalesforce(j$('#fileNameInput').val(), j$('#fileSuffix').val())
      .then(function (t) {
        Configuration.template = t;
        buttonApiName = Configuration.layoutActionName + buttonApiName;

        j$.each(j$('input[name="layoutCheckboxI"]:checked'), function () {
          var layout = _layoutMap[j$(this).val()];
          delete _layoutMap[j$(this).val()];
...
j$('#fileNamingRules').hover(function () {
  if (_toolTip) {
    j$('#namingHelp').hide();
    _toolTip = false;
  } else {
...
function prepareSendForSignature() {
  hideAll();
  Elements.spinner2.show();
  if (Configuration.template) {
    exportDocument('html')
